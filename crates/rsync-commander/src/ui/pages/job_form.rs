...
pub fn draw_job_form(f: &mut Frame, app: &App, area: Rect) {
    let form = match &app.overlays.job_form {
        Some(f) => f,
        None => return,
    };
    ...
    let fields: Vec<(&str, String)> = vec![
        ...
        ("Source Path", format_location(&form.job.transfer.source)),
        ("Destination Path", format_location(&form.job.transfer.destination)),
        ("Backup Mode", format_mode(&form.job.transfer.backup_mode)),
        ("Archive", bool_str(form.job.options.core_transfer.archive)),
        ("Compress", bool_str(form.job.options.core_transfer.compress)),
        ("Verbose", bool_str(form.job.options.output.verbose)),
        ("Delete", bool_str(form.job.options.file_handling.delete)),
        ("Dry Run", bool_str(form.job.options.core_transfer.dry_run)),
