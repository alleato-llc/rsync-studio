...
pub fn draw_settings(f: &mut Frame, app: &App, area: Rect) {
    ...
    let settings: Vec<(&str, String)> = vec![
        ("Log Directory", app.settings_state.log_directory.clone()),
        (
            "Max Log Age (days)",
            app.settings_state.max_log_age_days.to_string(),
        ),
        (
            "Max History/Job",
            app.settings_state.max_history_per_job.to_string(),
        ),
        (
            "Auto Trailing Slash",
            if app.settings_state.auto_trailing_slash {
                "Yes".to_string()
            } else {
                "No".to_string()
            },
        ),
        ("TUI Theme", app.settings_state.tui_theme.clone()),
    ];
    ...
    for (i, (label, value)) in settings.iter().enumerate() {
        ...
        let is_selected = i == app.settings_state.selected;
        let is_editing = app.settings_state.editing && is_selected;
        ...
        if is_editing {
            f.render_widget(
                TextInputWidget::new(&app.settings_state.edit_input)
