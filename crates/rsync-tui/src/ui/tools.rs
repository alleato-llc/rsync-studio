...
pub fn draw_tools(f: &mut Frame, app: &App, area: Rect) {
    ...
    let tab_line = Line::from(
        tabs.iter()
            .enumerate()
            .map(|(i, label)| {
                let style = if i == app.tools_state.active_tab {
                    Style::default()
...
fn draw_explainer(f: &mut Frame, app: &App, area: Rect) {
    ...
    f.render_widget(
        TextInputWidget::new(&app.tools_state.command_input)
    ...
    if let Some(ref err) = app.tools_state.explanation_error {
        f.render_widget(
    ...
    } else if let Some(ref explanation) = app.tools_state.explanation {
        let mut lines = vec![
...
fn draw_scrubber(f: &mut Frame, app: &App, area: Rect) {
    ...
    f.render_widget(input_block, chunks[0]);
    f.render_widget(
        TextInputWidget::new(&app.tools_state.scrub_input)
            .focused_style(Style::default().fg(app.theme.fg))
            .unfocused_style(Style::default().fg(app.theme.muted)),
        input_inner,
    );
