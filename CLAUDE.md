...
│   └── rsync-tui/          # Terminal UI: ratatui + crossterm + clap
...
# TUI
cargo run -p rsync-tui         # Run the terminal UI
cargo run -p rsync-tui -- list # List jobs (non-interactive)
cargo run -p rsync-tui -- run <job-id>  # Run single job (non-interactive)
cargo build -p rsync-tui --release      # Release build
```
...
- Do not add logic to `src-tauri/` or `crates/rsync-tui/` that belongs in `rsync-core` — keep both frontend crates as thin shells
...
- Do not add dependencies to `crates/rsync-tui/Cargo.toml` unless they are TUI-specific (ratatui, crossterm, clap)
