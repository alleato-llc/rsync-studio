"""Generate the Rsync Desktop app icon.
...
def main():
    ...
    draw.arc(arc_bbox, ARC1_START_ANGLE, ARC1_END_ANGLE,
             fill=GOLD, width=ARROW_THICKNESS)

    # Arc 2: CW from 345° to 135° in Pillow (visually covers bottom-right quadrants)
    draw.arc(arc_bbox, ARC2_START_ANGLE, ARC2_END_ANGLE,
             fill=GOLD, width=ARROW_THICKNESS)

    # --- Arrowheads ---
    # Arrow 1 tip at 165° (pointing CCW toward ~150° direction)
    draw_arrowhead(draw, CENTER, CENTER, ARROW_RADIUS, ARROW1_TIP_ANGLE,
                   ARROWHEAD_LENGTH, ARROWHEAD_HALF_WIDTH, GOLD)

    # Arrow 2 tip at 345° (pointing CCW toward ~330° direction)
    draw_arrowhead(draw, CENTER, CENTER, ARROW_RADIUS, ARROW2_TIP_ANGLE,
                   ARROWHEAD_LENGTH, ARROWHEAD_HALF_WIDTH, GOLD)
    ...
    font_size = 380
    try:
        font = ImageFont.truetype("/System/Library/Fonts/Supplemental/Verdana Bold.ttf", font_size)
    except Exception:
        try:
            font = ImageFont.truetype("/System/Library/Fonts/HelveticaNeue.ttc", font_size, index=8)
        except Exception:
